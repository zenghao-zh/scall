...
def register_fakequant_hooks(model):
    """Hook every FakeQuant in backbone to capture int8 values (before dequant)."""
    hook_outputs = {}
    hooks = []
    ...
    for bb_name, bb_module in model.backbone._modules.items():
        if not isinstance(bb_module, torch.nn.Sequential):
            continue
        # bb_module is Sequential wrapping LSTM + FakeQuant(s)
        for sub_name, sub_module in bb_module._modules.items():
            if isinstance(sub_module, FakeQuant):
                tag = f"backbone.{bb_name}.{sub_name}"

                def make_hook(name, fq_mod):
                    def hook_fn(module, inp, output):
                        scale = fq_mod.scale.to(dtype=output.dtype, device=output.device)
                        int8_vals = torch.round(output / scale).to(torch.int8)
                        hook_outputs[name] = int8_vals.detach().cpu()
                    return hook_fn

                hooks.append(sub_module.register_forward_hook(make_hook(tag, sub_module)))
...
def main():
    ...
    SAMPLE_INPUT = os.path.join(SCRIPT_DIR, "sample_input.pt")
    CONFIG_PATH = "/workspace/huada/task_results/lstm_ctc_crf_optimized_l9_6x_0214/config.toml"
    IO_QUANT_PATH = os.path.join(SCRIPT_DIR, "..", "caoyu", "layer_9_6x_io_quant_0303.pth")
    ACT_SCALES_PATH = os.path.join(SCRIPT_DIR, "..", "caoyu", "layer_9_6x_act_scales_0303.pth")
    DEVICE = "cuda:0"
    ...
    # ---- Load model (bf16 + FakeQuant + ManualLSTMRNN) ----
    print("[Loading quantized model (bf16)]")
    ...
    # ---- Register hooks on every FakeQuant ----
    hook_outputs, hooks = register_fakequant_hooks(model)
    print(f"[Hooks registered] {len(hooks)} FakeQuant hooks")

    # ---- Load sample_input.pt ----
    data = torch.load(SAMPLE_INPUT, map_location="cpu")
    ...
    # ---- Print & save int8 outputs ----
    print(f"\n[Captured {len(hook_outputs)} FakeQuant int8 outputs]")
    print(f"{'Name':<30} {'Shape':<25} {'min':>6} {'max':>6}")
    print("-" * 70)
    save_dict = {}
    for name, tensor in hook_outputs.items():
        print(f"{name:<30} {str(tuple(tensor.shape)):<25} "
              f"{tensor.float().min().item():>6.0f} {tensor.float().max().item():>6.0f}")
        save_dict[name] = tensor.numpy()

    out_path = os.path.join(SCRIPT_DIR, "fakequant_int8_outputs.npz")
    np.savez_compressed(out_path, **save_dict)
    print(f"\n[Saved] {out_path}  ({len(save_dict)} layers)")

    # ---- Decode using viterbi_guided_bidirectional_reshape ----
    print("\n[Running viterbi_guided_bidirectional_reshape decoding]")
    with torch.no_grad():
        paths, alphas_all, betas_all, idx, idx_T, Ms, alpha_max = \
            viterbi_guided_bidirectional_reshape(model.seqdist, scores, use_bfloat16=True)
    print(f"  paths shape={paths.shape}")
    print(f"  alphas_all shape={alphas_all.shape}")
    print(f"  betas_all  shape={betas_all.shape}")
    print(f"  idx        shape={idx.shape}")
    print(f"  idx_T      shape={idx_T.shape}")
    print(f"  Ms         shape={Ms.shape}")
    print(f"  alpha_max  shape={alpha_max.shape}")
    ...
    decode_dict = {
        "alphas_all": to_np(alphas_all),
        "betas_all":  to_np(betas_all),
        "idx":        to_np(idx),
        "idx_T":      to_np(idx_T),
        "Ms":         to_np(Ms),
        "alpha_max":  to_np(alpha_max),
    }
    decode_path = os.path.join(SCRIPT_DIR, "decode_intermediates.npz")
    np.savez_compressed(decode_path, **decode_dict)
    print(f"\n[Saved] {decode_path}")
...
def viterbi_guided_bidirectional_reshape(self, scores, use_bfloat16=True):
        ...
        return paths.T.to(torch.long), alphas_all, betas_all, idx, idx_T, Ms, alpha_max

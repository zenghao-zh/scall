...
from opencall.utils.mymultiprocessing import process_map
from opencall.utils.util import mean_qscore_from_qstring
from opencall.utils.util import chunk, stitch, batchify, unbatchify, permute
...
def basecall(
    ...
    scores = (
        (
            read,
            {"scores": stitch(v, chunksize, overlap, len(read.signal), model.stride)},
        )
        for read, v in scores
    )
    decoder = partial(
        decode,
        decode=model.decode,
...
def decode(scores, decode, beamsize=5, qscores=False, stride=1):
    ...
    sig_move = None
    if path is not None:
        sig_move = np.full(len(path) * stride, False)

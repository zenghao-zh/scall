...
from opencall.libs.bonito.multiprocessing import thread_iter
from opencall.libs.bonito.util import (
    chunk,
    stitch,
    batchify,
    unbatchify,
...
def basecall(model, reads, chunksize=4000, overlap=100, batchsize=32, reverse=False):
    ...
    results = thread_iter(
        (
            read,
            stitch_results(
                scores, end - start, chunksize, overlap, model.stride, reverse
            ),
        )
        for ((read, start, end), scores) in unbatchify(scores)
    )

    return thread_iter((read, fmt(model.stride, attrs)) for read, attrs in results)

...
def main():
    ...
    res_dir = f'/workspace/huada/task_results/encoder_only_{args.output_name}'
    ...
    # 3. get data
    train_loader, valid_loader = get_dataset_in_one_dir(args, encoder_only=True)
                ...
                with amp.autocast(enabled=args.use_amp):
                    for data_, targets_, lengths_ in zip(
                        *map(lambda t: t.chunk(grad_accum_split, dim=0), batch)
                        ...
                        scores_ = model(data_)
                        losses_ = model_orig.loss(scores_, targets_)

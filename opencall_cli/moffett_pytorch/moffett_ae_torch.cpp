...
enum class Op { Sigmoid, Tanh, Rcp };

torch::Tensor run_impl(torch::Tensor x, Op op) {
    ...
    if (x_contig.is_cuda()) {
        ...
        switch (op) {
            case Op::Sigmoid: moffett::sigmoid_forward_cuda(in_ptr, out_ptr, numel); break;
            case Op::Tanh:    moffett::tanh_forward_cuda(in_ptr, out_ptr, numel); break;
            case Op::Rcp:     moffett::rcp_forward_cuda(in_ptr, out_ptr, numel); break;
        }
    } else {
        ...
        at::parallel_for(0, numel, kGrainSize, [&](int64_t begin, int64_t end) {
            ...
            switch (op) {
                case Op::Sigmoid: moffett::sigmoid_forward_cpu(in_ptr + begin, out_ptr + begin, len); break;
                case Op::Tanh:    moffett::tanh_forward_cpu(in_ptr + begin, out_ptr + begin, len); break;
                case Op::Rcp:     moffett::rcp_forward_cpu(in_ptr + begin, out_ptr + begin, len); break;
            }
        });
    }
...
torch::Tensor moffett_sigmoid_forward(torch::Tensor x) {
    return run_impl(std::move(x), Op::Sigmoid);
}

torch::Tensor moffett_tanh_forward(torch::Tensor x) {
    return run_impl(std::move(x), Op::Tanh);
}

torch::Tensor moffett_rcp_forward(torch::Tensor x) {
    return run_impl(std::move(x), Op::Rcp);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("rcp_forward", &moffett_rcp_forward, "Moffett-style reciprocal (1/x) forward (CPU/CUDA, bf16)");
}

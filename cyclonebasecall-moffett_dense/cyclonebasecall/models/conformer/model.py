import torch
import numpy as np
from cyclonebasecall.models.common.nn import Module, Convolution, LinearCRFEncoder, Serial, Permute, from_dict
...
# from cyclonebasecall.models.conformer import ConformerEncoder
from cyclonebasecall.models.conformer.modules.encoder import ConformerEncoder
...
class seqdistModel(Module):
    ...
    def decode_batch(self, x):
        ...
        res = [self.seqdist.path_to_str(x) for x in tracebacks.cpu().numpy()]
        return res
...
class Model(seqdistModel):
    ...
    def __init__(self, config):
        ...
        if 'type' in config['encoder']: #new-style config
            ...
            encoder = custom_encoder(seqdist.n_base, seqdist.state_len, insize=config['input']['features'], **config['encoder'])

...
import pysam
import requests
from exceptions import (CamError, PathNotFoundError, InvalidMappingToolError, CamFileAccessModeError)
import json

# sys.path.append(os.path.dirname(os.path.abspath(__file__)))


def change_mode(folder_name):
    url = "http://192.168.0.106:9111/run_cmd"
    cmd_chmod = {"cmd": f"chmod -R 777 /mnt/seqdata/output_data/{folder_name}/data_for_analysing/"}
    headers = {'Content-Type': 'text/plain'}
    response = requests.request("POST", url, headers=headers, data=json.dumps(cmd_chmod))
    r = response.text
...
class Cam:
    ...
    def __init__(self, ref_file: str, query_file: str, mapping_tool: str = "minimap2"):
        if not os.path.isfile(ref_file):
            raise PathNotFoundError(f"Reference sequence file {ref_file} not found")

        if not os.path.isfile(query_file):
            raise PathNotFoundError(f"Query sequence file {query_file} not found")
    ...
    def mapping(self):
        ...
        self.folder_name = os.path.splitext(os.path.basename(self.query_file))[0]
        ...
        sam = Sam2(sam_file=self.sam_file, ref_file=self.ref_file, query_file=self.query_file,
...
class Mapping:
    ...
    def __init__(self, ref_file: str, query_file: str, index_file: str = None,
        ...
        if not os.path.isfile(ref_file) or not os.path.isfile(query_file):
            raise FileNotFoundError(f"Reference file {ref_file} or query file {query_file} not found")

        self.default_ref_name = os.path.splitext(os.path.basename(ref_file))[0]
        self.default_query_name = os.path.splitext(os.path.basename(query_file))[0]
        self.ref_file = ref_file
        self.query_file = query_file

        if not index_file or not os.path.isfile(index_file):
            self.idx_file = os.path.join(os.path.dirname(self.ref_file), self.default_ref_name + ".idx")
        else:
            self.idx_file = index_file

        if sam_file is None:
            self.sam_file = os.path.join(os.path.dirname(self.query_file), self.default_query_name + ".sam")
        else:
            self.sam_file = sam_file

        if bam_file is None:
            self.bam_file = os.path.join(os.path.dirname(self.query_file), self.default_query_name + ".bam")
        else:
    ...
    def index(self):
        if not os.path.isfile(self.idx_file):
            ...
            po = subprocess.Popen(cmd.split(), bufsize=-1, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            po.wait()
        ...
        for query_name in details:
            ...
            for dataset_name in detail:
                if dataset_name in {"cigar_string", "qvalues_string"}:
                    record[dataset_name] = detail[dataset_name][0].decode("utf-8")
                else:
...
class CamFileWriter:
    ...
    def write_summary(self):
        ...
        for attr in attrs:
            summary.attrs.create(name=attr, dtype=attrs[attr], data=self.df_summary.at[0, attr])
    ...
    def write_detail(self):
        ...
        for _, row in self.df_detail.iterrows():
            ...
            for attr in attrs:
                detail.attrs.create(name=attr, dtype=attrs[attr], data=row[attr])
            for name in datasets:
                if name in {"cigar_string", "qvalues_string"}:
                    detail.create_dataset(name=name, dtype=h5py.special_dtype(vlen=str), shape=(1,), data=row[name])
                    # print(detail[name][0])
                else:
                    """
                    the rest fields are:
                    ...
                    query_ins_indices
                    """
                    detail.create_dataset(name=name, dtype="int", shape=row[name].shape, data=row[name])
        self.cam_file.flush()
...
class Sam2:
    ...
    def __init__(self, sam_file: str, ref_file: str, query_file: str, mapping_tool: str = None,
                 ...
                 **kwargs):
        if not os.path.isfile(ref_file) or not os.path.isfile(query_file):
            raise FileNotFoundError("Reference file '%s' or query file '%s' not found" % (ref_file, query_file))

        if not os.path.isfile(sam_file):
            raise FileNotFoundError("Bam file '%s' not found" % (sam_file))
        self._df_detail = None
        self._df_summary = None
        self._ref_qvalues = None  # a 2d numpy array, it's row number is 5, and the column number is the length of reference sequence
    ...
    def df_detail(self):
        if self._df_detail is None:
            ...
            self._df_detail.eval("identity_rate = total_match / (total_ref_base + total_base_ins)", inplace=True)
            self._df_detail.eval("mismatch_rate = total_mismatch / (total_ref_base + total_base_ins)", inplace=True)
            self._df_detail.eval("insertion_rate = total_base_ins / (total_ref_base + total_base_ins)", inplace=True)
            self._df_detail.eval("deletion_rate = total_base_del / (total_ref_base + total_base_ins)", inplace=True)
    ...
    def fetch_aligned_read(self):
        for query_read in self.sam_obj:
            # ref_seq = self.get_ref_seq(ref_name=query_read.reference_name)
            if query_read.reference_name is None:
                assert query_read.flag == 4
                ref_seq = ""
            else:
                ref_seq = str(self.ref_seqs[query_read.reference_name].seq)
            aligned_read = AlignedRead2(query_read=query_read, ref_seq=ref_seq)
    ...
    def summary(self):
        ...
        query_match_qvalues = np.array(self.query_counter["query_match_qvalues"])
        query_mismatch_qvalues = np.array(self.query_counter["query_mismatch_qvalues"])
    ...
    def collect_ref_qvalues(self):
        for _, row in self.df_detail.iterrows():
            ...
            if len(ref_match_indices) > 0 and len(query_match_indices) > 0:
                self.ref_counter[ref_name][3, ref_match_indices] += query_qvales[query_match_indices]
            if len(ref_mismatch_indices) > 0 and len(query_mismatch_indices) > 0:
                self.ref_counter[ref_name][4, ref_mismatch_indices] += query_qvales[query_mismatch_indices]
    ...
    def collect_query_qvalues(self):
        for _, row in self.df_detail.iterrows():
            ...
            read_qvalue.group()
            self.query_counter["query_match_qvalues"].extend(read_qvalue.match_qvalues)
            self.query_counter["query_mismatch_qvalues"].extend(read_qvalue.mismatch_qvalues)
            self.query_counter["query_ins_qvalues"].extend(read_qvalue.ins_qvalues)
    ...
    def collect_q40(self):
        ...
        try:
            ...
            max_qvalue = 40
        query_q40_match = self._group_q40(qvalues=query_match_qvalues, max_qvalue=max_qvalue)
        query_q40_mismatch = self._group_q40(qvalues=query_mismatch_qvalues, max_qvalue=max_qvalue)
        query_q40_ins = self._group_q40(qvalues=query_ins_qvalues, max_qvalue=max_qvalue)
...
cdef class AlignedRead2:
    """ Aligned read from SAM/BAM file. """
    cdef public Cigar2 _cigar
    cdef public object _aligned_read
    cdef public str ref_name
    cdef public int ref_read_start_pos
    cdef public str ref_read
    cdef public str query_name
    cdef public str query_read
    cdef public int query_length
    cdef public int query_alignment_length
    cdef public bint is_unmapped
    cdef public int flag
    cdef public int mapq
    cdef public str cigar_string
    cdef public str qvalues_string
    cdef public np.ndarray qvalues
    cdef public str folder_name
    cdef public str channel_num
    cdef public double read_start_time
    cdef public double read_end_time
    cdef public int read_start_index
    cdef public int read_end_index
    def __cinit__(self, str ref_seq, query_read):
        self._cigar = None
        self.init_attributes(ref_seq=ref_seq, sam_read=query_read)

    cdef void init_attributes(self, str ref_seq, sam_read):
        ...
        # print(self.query_alignment_length)
        ...
        self.qvalues = np.array(sam_read.query_qualities) if sam_read.query_qualities else np.array([])
    ...
    cdef str set_ref_read(self, ref_seq, query_read):
    ...
    cdef void split_query_name(self):
    ...
    def coverage(self):
        try:
            _coverage = float(self.query_alignment_length) / float(self.query_length)
        except ZeroDivisionError:
            # print("ZeroDivisionError")
            _coverage = 0.0
        # print(f'{self.query_alignment_length} / {self.query_length} = {_coverage}')
        return _coverage
...
cdef class Cigar2:
    ...
    cdef str ref_read
    cdef str query_read
    cdef list ref_operations
    cdef list query_operations
    cdef str cigar_ext
    cdef str ref_cigar
    cdef str query_cigar
    cdef public dict stats
    cdef public np.ndarray ref_match_indices
    cdef public np.ndarray query_match_indices
    cdef public np.ndarray ref_mismatch_indices
    cdef public np.ndarray query_mismatch_indices
    cdef public np.ndarray ref_delete_indices
    cdef public np.ndarray query_insert_indices
    cdef public np.ndarray query_clip_indices

    def __cinit__(self, str cigar_string, str ref_read, str query_read):
        ...
        # self.ref_match_indices = np.array([])  # np.array, the same below
        # self.query_match_indices = np.array([])
        #
        # self.ref_mismatch_indices = np.array([])
        # self.query_mismatch_indices = np.array([])
        #
        # self.query_insert_indices = np.array([])
        # self.ref_delete_indices = np.array([])
        #
        # self.query_clip_indices = np.array([])
    ...
    cdef void parse(self):
        pattern = re.compile("([0-9]+[a-zA-Z=])")
        elems = pattern.findall(self.cigar)
        for elem in elems:
            times = int(elem[:-1])
            op = elem[-1]
    ...
    cdef np.ndarray _find_indices(self, string: str, operations: List[str]):
        indices = np.array([i for i, s in enumerate(string) if s in operations])
        return indices

    cdef dict count(self):
...
class Quality2:
    ...
    def __init__(self, qvalue_seq, match_indices, mismatch_indices, ins_indices):

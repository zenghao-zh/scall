...
import numpy as np
import cyclonebasecall.bonito.reader
from tqdm import tqdm
from pod5_format import CombinedReader


class Read(cyclonebasecall.bonito.reader.Read):
    ...
    def __init__(self, read, filename, meta=False):
        ...
        trim_start, _ = cyclonebasecall.bonito.reader.trim(scaled[:8000])
        ...
        if len(scaled) > 8000:
            self.med, self.mad = cyclonebasecall.bonito.reader.med_mad(scaled)
            self.mad = max(1.0, self.mad)
            self.signal = (scaled - self.med) / self.mad
        else:
            self.signal, (self.med, self.mad) = cyclonebasecall.bonito.reader.norm_by_noisiest_section(scaled, return_medmad=True)

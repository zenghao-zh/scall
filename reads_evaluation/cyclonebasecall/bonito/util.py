...
def load_symbol(config, symbol):
    ...
    m_name = config['model']['package'].split(".")[-1]
    package_name = "cyclonebasecall.models.{}".format(m_name)
    imported = import_module(package_name)
...
def replace_lstm_with_manual(model):
    ...
    """
    from cyclonebasecall.models.common.nn import LSTM, ManualLSTMRNN
    replaced = 0
...
def load_quant_model(dirname, device, io_quant_path, act_scales_path, half=None, bf16=False, bitwidth=8):
    ...
    if bf16:
        ...
        if half:
            model = model.half()
            print(f"[load_quant_model] Model converted to float16")
    elif half:
        model = model.half()
        print(f"[load_quant_model] Model converted to float16")
    else:
        print(f"[load_quant_model] Model kept in float32")
    model.eval()
    model.to(device_obj)
    print(f"[load_quant_model] Model on {device_obj}, dtype={next(model.parameters()).dtype}")
    return model

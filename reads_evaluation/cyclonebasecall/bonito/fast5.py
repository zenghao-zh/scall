...
import numpy as np
import cyclonebasecall.bonito.reader as bonito_reader
from tqdm import tqdm
from dateutil import parser
from ont_fast5_api.fast5_interface import get_fast5_file


class Read(bonito_reader.Read):
    ...
    def __init__(self, read, filename, meta=False):
        ...
        trim_start, _ = bonito_reader.trim(scaled[:8000])
        ...
        if len(scaled) > 8000:
            self.med, self.mad = bonito_reader.med_mad(scaled)
            self.mad = max(1.0, self.mad)
            self.signal = (scaled - self.med) / self.mad
        else:
            self.signal, (self.med, self.mad) = bonito_reader.norm_by_noisiest_section(scaled, return_medmad=True)
